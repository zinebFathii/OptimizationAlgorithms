...
x1=rand()*randi([-3 3],1);                                          %x değişkeni +3~-3
x2=rand()*randi([-2 2],1);                                     %y değişkeni -2~+2
 
fitness=(abs(x1)+abs(x2))*exp(-1*(sin((x1)^2)+sin((x2)^2)));    %fitness fonksiyon hesabı
 
prt(i,1)=x1;                                                              %i. x değeri
prt(i,2)=x1;                                                               %current x
prt(i,3)=x2;                                                               %i. y değeri
prt(i,4)=x2;                                                               %current y
...
Vy(i)=0;                                                %y vektörünün başlangıç hızları
 ...
 while z<1000                                                %iterasyon başlangıcı
    ...
    for i=1:20
        ...
        prt(i,5)=(abs(prt(i,2))+abs(prt(i,4)))*exp(-1*(sin(prt(i,2)^2)+sin(prt(i,4)^2)));%yeni lokasyon fitness fonksiyon hesabı
